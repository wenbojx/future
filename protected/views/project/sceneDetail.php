<div id="js_cantain_box" class="col-main">
        ...
        <div id="js_data_list_outer" class="page-detail">
            ...
            <div id="detail-pano-camera" class="pano-detail-tip">
                ...
                <div>
                    <a class="scene_detail_save" href="javascript:" onclick="save_camera_detail(<?=$datas['scene_id']?>)">保存摄像机信息</a>
    ...
    <div class="col-sub">
        <div style="text-align:center; margin:10px 0 0 0;">
            <a onclick="loadModalWin('showmodel','<?=$this->createUrl('/project/scene/uploadPhoto/show/', array('scene_id'=>$datas['scene_id']))?>', '上传', 605, 450)" style="" class="button btn-orange" href="javascript:;">上传</a>
        </div>
        ...
        <div style="text-align:center; margin:10px 0 0 0;">
        	<a onclick="publish_scene(<?=$datas['scene_id']?>,2)" class="button btn-orange" href="javascript:;">发布</a>
            <a onclick="publish_scene(<?=$datas['scene_id']?>,1)" class="button btn-orange" href="javascript:;">下线</a>
        	<br><span id="publish_scene_msg"></span>
    </div>
<script type="text/javascript">

var player_url = baseUrl+'/pages/salado/Player.swf';
var scene_xml_url = '<?=$this->createUrl('/salado/index/a/', array('id'=>$datas['scene_id'], 'from'=>'admin'))?>';
var id = 'scene_box';
load_scene(id, scene_xml_url);
var position_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'position', 'scene_id'=>$datas['scene_id']))?>';
var basic_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'basic', 'scene_id'=>$datas['scene_id']))?>';
var preview_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'preview', 'scene_id'=>$datas['scene_id']))?>';
var thumb_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'thumb', 'scene_id'=>$datas['scene_id']))?>';
//var view_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'view', 'scene_id'=>$datas['scene_id']))?>';
var hotspot_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'hotspot', 'scene_id'=>$datas['scene_id']))?>';
var button_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'button', 'scene_id'=>$datas['scene_id']))?>';
var map_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'map', 'scene_id'=>$datas['scene_id']))?>';
var navigat_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'navigat', 'scene_id'=>$datas['scene_id']))?>';
var radar_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'radar', 'scene_id'=>$datas['scene_id']))?>';
var html_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'html', 'scene_id'=>$datas['scene_id']))?>';
var rightkey_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'rightkey', 'scene_id'=>$datas['scene_id']))?>';
var link_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'link', 'scene_id'=>$datas['scene_id']))?>';
var flare_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'flare', 'scene_id'=>$datas['scene_id']))?>';
var action_url = '<?=$this->createUrl('/project/scene/config/v/', array('t'=>'action', 'scene_id'=>$datas['scene_id']))?>';
bind_scene_btn();
</script>
